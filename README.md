...
### JWT (JSON Web Token) Authentication
JWT Authentication uses a JSON Web Token, which is a signed and encrypted token containing claims about the user. The token is sent in the `Authorization` header as `Bearer <jwt>`.
